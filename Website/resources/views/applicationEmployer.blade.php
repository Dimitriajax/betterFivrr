    ...
    <div class="max-w-4xl mx-auto">
        ...
        <section class="flex justify-between md:flex-row flex-col md:px-0 px-8">
            <div class="py-8 flex items-center gap-6">
                <a href="{{ route('user', $post['company_name'] ) }}">
                    <img src="{{ $post['company_logo'] }}" alt="" class="rounded-full h-24 w-24 object-cover">
                </a>
                <div class="flex flex-col items-start gap-4 justify-start">
                    <p class="text-3xl text-white font-bold"><span class="text-hotpink">@</span>{{$post["company_name"]}}</p>
                    <p class="text-xl text-lightgrey font-bold">{{$post["address"]}}</p>
                </div>
            </div>
            <div class="flex items-center">
                <button class="text-lightgrey border-blue border-2 rounded-lg py-1 px-2">stuur {{ $post['company_name'] }} een bericht</button>
            </div>
        </section>
        <section class="pb-4 md:px-0 px-8 md:pt-0 pt-12">
            <div class="flex justify-between md:flex-row flex-col">
                <div class="flex flex-col md:w-3/5">
                    <h2 class="text-hotpink text-3xl font-bold py-6">Beschrijving vacature</h2>
                ...
                <div class="flex flex-col w-64">
                    <h2 class="text-blue text-3xl font-bold py-6">Informatie</h2>
                    <div class="bg-[#202020] text-white  py-5 px-5 rounded-xl ">
                        <h4 class="font-semibold text-lg">€ {{ $post["price"] }}</h4>
                        <li>
                            <ul></ul>
                        </li>
                    </div>
